...
export default function App(){
 ...
 return <>{loading&&<LoadingScreen motion={motion} onOpening={beginOpening} onComplete={completeLoading}/>}<a className="skip-link" href="#main">Skip to content</a><header className="site-header" inert={!ready}><a className="wordmark" href="#intro" aria-label="Kiran, introduction">KIRAN</a><nav aria-label="Main navigation"><a href="#intro">Intro</a><a href="#journey">Journey</a><a href="#work">Work</a><a href="#contact">Contact <span>↗</span></a></nav><button className="motion-toggle" onClick={()=>setEnabled(v=>!v)} aria-pressed={motion} title={reduced?'Your system requests reduced motion':'Toggle animation'}>Motion {motion?'on':'off'} <span aria-hidden="true">{motion?'◉':'○'}</span></button></header>
 ...
 <Toolkit/>
 <Interests/>
 <Contact/></main><footer><a className="wordmark" href="#intro">KIRAN</a><span>Kiran Kishore · © {new Date().getFullYear()}</span><a href="#intro">Back to top ↑</a></footer><CaseStudy project={project} onClose={close}/></>;
}
